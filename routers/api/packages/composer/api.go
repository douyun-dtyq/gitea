...
type PackageVersionMetadata struct {
	*composer_module.Metadata
	...
	PublishedTime     time.Time      `json:"published-time"`
...
func loadDevBranchComposerMetadata(ctx *context.Context, repo *repo_model.Repository, branch string) (*git.Commit, map[string]any) {
	gitRepo, err := git.OpenRepository(ctx, repo.CodeStorageRepo())
	if err != nil {
	...
	content, err := commit.GetFileContent(ctx, gitRepo, "composer.json", 10*1024*1024)
	if err != nil {
